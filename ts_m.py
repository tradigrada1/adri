    ...
    if user or not user:
        ...
        if message == 'a' or message == 'A':
            if 'Publicar Nota/Pauta?' in user_pr['last']:
                ...
                reply = {'name':name,
                'output':[data_msgs['need_act_2'],data_msgs['need_act_3'],data_msgs['need_act_4']],
                'status':status}
                print('reply 22')
...
# alli = show_all_users()
# ur = alli[0]['message_details']['image']
...
#*****************************
def post_now(image,title,text):
    print("posted")
    ##creat_post.creat_p(image,title,text)
#*****************************
